...
### 2026-05-30 — TASK_08: Historical Redistricting Comparison (Phase 4)

Added CD116 vs CD119 district boundary comparison mode with an opacity slider.

Pre-implementation research: queried the Census TIGER Legislative MapServer (`?f=json`) to enumerate all layers before writing any code. Layer 12 = "116th Congressional Districts" with field `CD116` — the task spec guessed layer 10, which is actually "2024 State Legislative Districts - Lower".

Backend changes: `integrations/census.py` gains `HISTORICAL_CD_LAYER = 12` constant, `get_historical_district_data_dir()` for settings-aware path resolution, `load_local_historical_districts(state)` to serve pre-built files, and `fetch_historical_congressional_districts(state)` which hits TIGER layer 12 with `outFields=GEOID,CD116,NAME,STATE` and 0.01° geometry simplification; `settings.py` gains `HISTORICAL_DISTRICT_DATA_DIR` env var override; new `management/commands/build_historical_district_data.py` with `--states` and `--overwrite` flags, mirroring `build_district_data.py`; new `historical_district_data/.gitkeep` directory; `views.py` adds `HistoricalDistrictView` at `GET /api/v1/districts/historical/?state=CA` with the same local-file → 7-day cache → live Census fallback chain (503 when fallback disabled); `urls.py` registers the route. New `tests_historical_redistricting.py` with 17 tests: 400 for missing/invalid state, local-file hit, cache hit, live fallback enabled/disabled, Census failure → 500, command create/skip/overwrite/failure, and census helper unit tests including a check that layer 12 is used in the request URL.

Frontend changes: `redistrictingMode: boolean` and `redistrictingSliderValue: number` (0–100) added to `MapState` type and `mapStore.ts` with setters; `fetchHistoricalDistricts(state)` added to `api/representatives.ts`; new `RedistrictingOverlay.tsx` — a Mapbox Source+Layer component that fetches CD116 GeoJSON for the selected state (module-level session cache to avoid refetches on remount), renders amber/orange fill and dashed-line layers, opacity = `(100 - sliderValue) / 100`; new `RedistrictingSlider.tsx` + `RedistrictingSlider.css` — glassmorphism control panel positioned above the NavigationControl, labeled "2013–2023 (CD116)" on the left and "2023–present (CD119)" on the right with full ARIA attributes; `DistrictOverlay.tsx` gains an `opacityScale` prop so current district opacity scales as `0.15 × scale` (fill) and `1 × scale` (line) — passed as `redistrictingSliderValue / 100` from `RepMap.tsx` in redistricting mode; `RepMap.tsx` reads `redistrictingMode` and `redistrictingSliderValue` from the store, passes `opacityScale` to `DistrictOverlay`, and renders `RedistrictingOverlay` when `redistrictingMode && viewLevel === 'federal' && zoom >= 5 && selectedStateCode`; `NavBar.tsx` adds an `onRedistrictingClick` prop and a "Redistricting" toggle button with amber active styling (`navbar-theme-btn--active`); `NavBar.css` adds the active state rule; `App.tsx` reads `redistrictingMode` and the setters from the store, wires the NavBar callback (toggle off resets slider to 100), and renders `<RedistrictingSlider />` inside the map area when mode is on and view is federal.

Verification: `python manage.py test` — 162 tests, all green. `npx tsc --noEmit` and `npm run build` clean (expected Mapbox bundle-size warning only).

### 2026-05-30 — TASK_09: PWA + Offline Mode (Phase 4)

Converted RepMap into a Progressive Web App with offline support.

`vite-plugin-pwa` added as a dev dependency. The plugin runs in `generateSW` mode with Workbox, activating only on production builds (`devOptions: { enabled: false }`). Pre-caching covers the full app shell (JS/CSS/HTML/fonts/icons). Runtime caching uses `StaleWhileRevalidate` for `/api/v1/representatives/` (24h), `CacheFirst` for `/data/national_districts.json` (30 days) and Google Fonts webfonts (30 days), `StaleWhileRevalidate` for Google Fonts stylesheets, `NetworkFirst` for `/api/v1/config/`, and `CacheFirst` for the Mapbox GL CSS CDN asset. Mapbox tile/sprite/glyph API requests and Mapbox events are explicitly `NetworkOnly` to comply with Mapbox TOS. `navigateFallbackDenylist: [/^\/api/]` prevents the SPA offline fallback from intercepting API navigation requests.

Two PWA icons (192×192 and 512×512) were generated programmatically in a brand gradient (blue `#3b82f6` → purple `#8b5cf6`) on a dark `#0f172a` background. `public/manifest.json` covers all required PWA manifest fields. `index.html` gains the manifest link, `theme-color`, Apple Web App meta tags, and `apple-touch-icon`. An offline status banner (amber, fixed at top, `role="status"` + `aria-live="polite"`) is added to `App.tsx` driven by `navigator.onLine` state with `online`/`offline` event listeners; it appears automatically when connectivity is lost and auto-dismisses on reconnection.

Build output: `dist/sw.js`, `dist/workbox-*.js`, `dist/manifest.json`, `dist/icons/icon-{192,512}.png`. `npx tsc --noEmit` and `npm run build` both clean (expected Mapbox bundle-size warning only).

Phase 4 feature work complete.
